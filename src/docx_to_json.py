    ...
    for line in text:
        ...
        if title_match:
            ...
            current_row = {
                ...
                'title_2': f"<b>{title_index}-Title:</b> {title_match.group(1)}",
                'content': ''
            }
        ...
        if authors_match:
            title_2_parts.append(f"<b>Authors:</b> {authors_match.group(1)}")

        if source_match:
            title_2_parts.append(f"<b>Source:</b> {source_match.group(1)}")
...
# Function to export parsed data to a JSON file
...
def docx_to_json(input_file, output_file):
    ...
    # Step 3: Export the parsed data to a JSON file
    export_to_json(parsed_data, output_file)

# Example usage with corrected paths
input_file = '../docx_files/2024_ruminant_science_june.docx'  # Corrected file name and path
output_file = '../output/output_file.json'  # Ensure 'output' directory exists
